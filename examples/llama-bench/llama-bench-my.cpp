...
static void test_prompt(llama_context * ctx, int n_prompt, int n_past, int n_batch, int n_threads) {
    ...
    while (n_processed < n_prompt) {
        ...
        tokens[0] = n_processed == 0 && llama_add_bos_token(model) ? llama_token_bos(model) : std::rand() % n_vocab; // embedded tokens
...
int main(int argc, char ** argv) {
    ...
    std::vector<cmd_params_instance> params_instances = get_cmd_params_instances(params); // three n_gen n_prompt gen+prompt
    ...
    for (const auto & inst : params_instances) {
        ...
        if (!lmodel || !prev_inst || !inst.equal_mparams(*prev_inst)) { // model loading related setting 6 + 1
            if (lmodel) {
                llama_free_model(lmodel);
            }

            uint64_t t_start_load = get_time_ns();
            lmodel = llama_load_model_from_file(inst.model.c_str(), inst.to_llama_mparams()); // model tag with model parameter
            uint64_t t_load_ns = get_time_ns() - t_start_load;
            double t_load_s = t_load_ns / 1e9; // Convert nanoseconds to seconds
            fprintf(stdout, "Test Model Load time: %.9f s\n", t_load_s);
            
            if (lmodel == NULL) {
                fprintf(stderr, "%s: error: failed to load model '%s'\n", __func__, inst.model.c_str());
                return 1;
            }
            prev_inst = &inst; // update input 
        }
